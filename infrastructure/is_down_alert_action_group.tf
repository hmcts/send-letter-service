data "azurerm_key_vault_secret" "source_alert_email_secret" {
  name      = "send-letter-failure-email"
  vault_uri = "${local.permanent_vault_uri}"
}

resource "azurerm_key_vault_secret" "alert_email_secret" {
  name      = "send-letter-failure-email"
  value     = "${data.azurerm_key_vault_secret.source_alert_email_secret.value}"
...
module "is-down-action-group" {
  ...
  email_receiver_address = "${data.azurerm_key_vault_secret.source_alert_email_secret.value}"
}
