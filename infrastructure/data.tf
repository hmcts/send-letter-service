locals {
  ...
  app_aks_network_name    = var.env == "aat" ? local.aat_cft_vnet_name : "core-${local.aks_env}-vnet"
  app_aks_network_rg_name = var.env == "aat" ? local.aat_cft_vnet_resource_group : "aks-infra-${local.aks_env}-rg"
}
