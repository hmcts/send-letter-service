...
public class MarkLettersPostedTask {
    ...
    private void markAsPosted(LetterPrintStatus letterPrintStatus, String reportFileName) {
        Optional<LetterStatus> status = repo.findStatusById(letterPrintStatus.id);
        if (status.isPresent()) {
            if (status.get() == LetterStatus.Uploaded) {
                repo.markAsPosted(letterPrintStatus.id, letterPrintStatus.printedAt.toLocalDateTime());
